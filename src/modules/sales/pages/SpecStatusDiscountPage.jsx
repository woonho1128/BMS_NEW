...
export function SpecStatusDiscountPage() {
  ...
  const paginatedRows = useMemo(() => {
    const list = activeTab === 'detail' ? filteredRows : summaryRows;
    const start = (currentPage - 1) * itemsPerPage;
    return list.slice(start, start + itemsPerPage);
  }, [filteredRows, summaryRows, activeTab, currentPage]);

  const totalPages = Math.ceil((activeTab === 'detail' ? filteredRows.length : summaryRows.length) / itemsPerPage);
    ...
    >
      <div className={styles.page}>
        <div className={styles.tabs} role="tablist">
          <button type="button" role="tab" aria-selected={activeTab === 'detail'} className={activeTab === 'detail' ? styles.tabActive : styles.tab} onClick={() => { setActiveTab('detail'); setCurrentPage(1); }}>
            SPEC-현황(할인포함)
          </button>
          <button type="button" role="tab" aria-selected={activeTab === 'summary'} className={activeTab === 'summary' ? styles.tabActive : styles.tab} onClick={() => { setActiveTab('summary'); setCurrentPage(1); }}>
            SPEC-현황(SUMMARY)
          </button>
        </div>
                ...
                <table className={styles.table}>
                  <thead>
                    <tr>
                      ...
                      <th className={styles.th}>품목별 수주유형</th>
                      <th className={styles.th}>사업분류</th>
                      <th className={styles.th}>지역</th>
                      <th className={styles.th}>현장명</th>
                      <th className={styles.th}>대리점</th>
                      <th className={styles.th} style={{ textAlign: 'right' }}>적용세대수</th>
                      <th className={styles.th}>납기예정</th>
                      <th className={styles.th}>수주일자</th>
                      <th className={styles.th}>SET번호</th>
                      <th className={styles.th}>품목그룹</th>
                      <th className={styles.th}>품목</th>
                      <th className={styles.th} style={{ textAlign: 'right' }}>수량</th>
                      <th className={styles.th} style={{ textAlign: 'right' }}>건설사납품단가</th>
                      <th className={styles.th} style={{ textAlign: 'right' }}>대리점납품단가</th>
                      <th className={styles.th} style={{ textAlign: 'right' }}>차이금액</th>
                      <th className={styles.th} style={{ textAlign: 'right' }}>금액</th>
                      <th className={styles.th} style={{ textAlign: 'right' }}>중량(KG)</th>
                      <th className={styles.th} style={{ textAlign: 'right' }}>총무게(KG)</th>
                      <th className={styles.th} style={{ textAlign: 'right' }}>총무게(TON)</th>
                      <th className={styles.th} style={{ textAlign: 'right' }}>톤당 단가</th>
                      <th className={styles.th}>비고</th>
                      <th className={styles.th}>수정일</th>
                      <th className={styles.th}>스펙구분</th>
                      <th className={styles.th}>납품구분</th>
                      <th className={styles.th}>진행사항</th>
                      <th className={styles.th}>스펙번호</th>
                      <th className={styles.th} style={{ textAlign: 'right' }}>할인(%)</th>
                      <th className={styles.th} style={{ textAlign: 'right' }}>세대원가/매입단가</th>
                      <th className={styles.th} style={{ textAlign: 'right' }}>총원가</th>
                      <th className={styles.th} style={{ textAlign: 'right' }}>총세대수</th>
                    </tr>
                  </thead>
                  <tbody>
                    {paginatedRows.length === 0 ? (
                      <tr>
                        <td colSpan={34} className={styles.empty}>
                          조회 결과가 없습니다.
                        </td>
                      </tr>
                    ) : (
                      paginatedRows.map((row) => (
                        <tr key={row.key} className={styles.tr}>
                          ...
                          <td className={styles.td}>{row.builder}</td>
                          <td className={styles.td}>{row.orderType}</td>
                          <td className={styles.td}>{row.itemOrderType}</td>
                          <td className={styles.td}>{row.bizClass}</td>
                          <td className={styles.td}>{row.region}</td>
                          <td className={styles.td}>{row.siteName}</td>
                          <td className={styles.td}>{row.agency}</td>
                          <td className={styles.tdNum}>{formatNum(row.households)}</td>
                          <td className={styles.td}>{row.dueDate}</td>
                          <td className={styles.td}>{row.orderDate}</td>
                          <td className={styles.td}>{row.setPartNo}</td>
                          <td className={styles.td}>{row.itemGroup}</td>
                          <td className={styles.td}>{row.item}</td>
                          <td className={styles.tdNum}>{formatNum(row.qty)}</td>
                          <td className={styles.tdNum}>{formatNum(row.builderPrice)}</td>
                          <td className={styles.tdNum}>{formatNum(row.agencyPrice)}</td>
                          <td className={styles.tdNum}>{formatNum(row.diffAmount)}</td>
                          <td className={styles.tdNum}>{formatNum(row.totalAmount)}</td>
                          <td className={styles.tdNum}>{formatNum(row.weightKg)}</td>
                          <td className={styles.tdNum}>{formatNum(row.totalWeightKg)}</td>
                          <td className={styles.tdNum}>{formatNum(row.totalWeightTon)}</td>
                          <td className={styles.tdNum}>{formatNum(row.pricePerTon)}</td>
                          <td className={styles.td}>{row.remarks}</td>
                          <td className={styles.td}>{row.modifyDate}</td>
                          <td className={styles.td}>{row.specType}</td>
                          <td className={styles.td}>{row.deliveryType}</td>
                          <td className={styles.td}>{row.progress}</td>
                          <td className={styles.td}>{row.specNo}</td>
                          <td className={styles.tdNum}>{row.discount}</td>
                          <td className={styles.tdNum}>{formatNum(row.stdCost)}</td>
                          <td className={styles.tdNum}>{formatNum(row.totalCost)}</td>
                          <td className={styles.tdNum}>{formatNum(row.totalHouseholds)}</td>
                        </tr>
                      ))
                    )}
                  </tbody>
        ...
        {activeTab === 'summary' && (
          <div className={styles.summarySection}>
            ...
            <div className={styles.tableTop}>
              <span className={styles.tableCount}>조회 결과: {summaryRows.length} 건</span>
            </div>
            <div className={styles.tableContainer}>
              <div className={styles.tableWrap}>
                <table className={styles.table}>
                  <thead>
                    <tr>
                      ...
                      <th className={styles.th}>현장명</th>
                      <th className={styles.th}>대리점명</th>
                      <th className={styles.th}>건설사명</th>
                      <th className={styles.th}>SPEC 납품예정일</th>
                      <th className={styles.th} style={{ textAlign: 'right' }}>ERP-금액</th>
                      <th className={styles.th} style={{ textAlign: 'right' }}>DECO-금액</th>
                      <th className={styles.th} style={{ textAlign: 'right' }}>차이금액</th>
                    </tr>
                  </thead>
                  <tbody>
                    {paginatedRows.length === 0 ? (
                      ...
                      paginatedRows.map((row) => (
                        <tr key={row.key} className={styles.tr}>
                          ...
                          <td className={styles.td}>{row.builder}</td>
                          <td className={styles.td}>{row.dueDate}</td>
                          <td className={styles.tdNum}>{formatNum(row.erpAmount)}</td>
                          <td className={styles.tdNum}>{formatNum(row.decoAmount)}</td>
                          <td className={styles.tdNum}>{formatNum(row.diffAmount)}</td>
                        </tr>
                      ))
                    )}
