...
export function TripReportDetailPage() {
  const { id } = useParams();
  ...
  if (!report || report.type !== REPORT_TYPE.TRIP) {
    return (
      <PageShell path="/sales/report" title="출장보고 상세">
        <p>출장보고를 찾을 수 없습니다.</p>
      ...
      <div className={styles.page}>
        <Card title="출장보고" className={styles.sectionCard}>
          <CardBody>
            ...
            <div className={styles.readOnlyField}>
              <span className={styles.readOnlyLabel}>출장지 / 목적 / 동행인</span>
              <span className={styles.readOnlyValue}>
            ...
            {report.costSummary && (
              <div className={styles.readOnlyField}>
                <span className={styles.readOnlyLabel}>비용 요약</span>
