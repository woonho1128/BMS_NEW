...
export function OrgDetailPanel({ org, onAddChild, onEdit, onDelete, onSave, onCancel }) {
  ...
  const [formData, setFormData] = useState({ code: '', name: '', parentId: '', order: 1, isActive: true });
  ...
  const breadcrumb = useCallback(() => {
    ...
    while (current) {
      ...
      current = current.parentId ? allOrgs.find((o) => o.id === current.parentId) : null;
    }
    return chain;
  }, [org, allOrgs])();
      ...
      <Card className={styles.emptyCard}>
        <CardBody>
          <div className={styles.emptyState}>
            <p className={styles.emptyText}>좌측 트리에서 조직을 선택해 주세요.</p>
            <p className={styles.emptyText}>또는 상단 [+ 조직 추가] 버튼을 눌러 새 조직을 생성하세요.</p>
          </div>
        </CardBody>
      </Card>
    );
          ...
          <div className={styles.tabContent}>
            ...
            <div className={styles.formCard}>
              <div className={styles.formGrid}>
                <div className={styles.field}>
                  ...
                  <input type="text" className={styles.input} value={formData.code} onChange={(e) => setFormData((prev) => ({ ...prev, code: e.target.value }))} disabled={!isEditing} placeholder="조직코드를 입력하세요" />
                </div>
                <div className={styles.field}>
                  <label className={styles.label}>조직명 *</label>
                  <input type="text" className={styles.input} value={formData.name} onChange={(e) => setFormData((prev) => ({ ...prev, name: e.target.value }))} disabled={!isEditing} placeholder="조직명을 입력하세요" />
                </div>
                <div className={styles.field}>
                  <label className={styles.label}>상위조직</label>
                  <select className={styles.select} value={formData.parentId} onChange={(e) => setFormData((prev) => ({ ...prev, parentId: e.target.value }))} disabled={!isEditing}>
                    <option value="">상위조직 선택 (없음)</option>
                    {allOrgs.filter((orgOption) => orgOption.id !== org.id).map((orgOption) => (
                ...
                <div className={styles.field}>
                  ...
                  <input type="number" className={styles.input} value={formData.order} onChange={(e) => setFormData((prev) => ({ ...prev, order: parseInt(e.target.value, 10) || 1 }))} disabled={!isEditing} min="1" />
                </div>
                <div className={styles.field}>
                  <label className={styles.label}>사용여부</label>
                  <select className={styles.select} value={formData.isActive ? 'true' : 'false'} onChange={(e) => setFormData((prev) => ({ ...prev, isActive: e.target.value === 'true' }))} disabled={!isEditing}>
            ...
            <div className={styles.actions}>
              {isEditing ? (
                ...
                <>
                  <Button variant="secondary" onClick={() => onAddChild && onAddChild()}>하위 조직 추가</Button>
        ...
        {activeTab === 'members' && (
          <div className={styles.tabContent}>
            <div className={styles.searchBox}>
              <input type="text" className={styles.searchInput} placeholder="조직원 검색" value={memberSearchQuery} onChange={(e) => setMemberSearchQuery(e.target.value)} />
            </div>
